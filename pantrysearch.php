<?php
    ...
    $pantrysearch = "SELECT DISTINCT `RecipeName` FROM `recipe-ingredients` where Ingredient IN (Select Ingredient FROM `user-pantry` where EmailID LIKE '$email') ";
    $pantryresult= mysqli_query($conn, $pantrysearch);
	echo"
<!DOCTYPE html>
<html lang='en'>
...
<body>
    ...
    <h3 style='margin:50px;'>Showing results for my pantry</h3>
    <div class='row' ><br><br>";
        
    if ($pantryresult->num_rows > 0) {
        while($row = mysqli_fetch_assoc($pantryresult)){
            $pantryrecipe=$row['RecipeName'];
            $getrecipes="SELECT RecipePicture,RecipeName,Rating,SkillLevel,CookTime,PrepTime FROM `recipe` WHERE RecipeName LIKE '$pantryrecipe'";
            $recipes=$conn->query($getrecipes);
            while($r = mysqli_fetch_assoc($recipes)){
                echo"<form action=\"\" method=\"POST\">
                <div class='col-lg-4'>
                    <div class='card'>
                        <img src='".$r['RecipePicture']."' alt='".$r['RecipeName']."' style='width:400px; height:300px;'>
                        <div class='container'>
                            <div class='card-content'>
                                <h4><strong><a href=\"RecipePage.php?id=".$r['RecipeName']."\">".$r['RecipeName']." </a></strong></h4>
                                <p class='desc-content'><strong>Rating : </strong>".$r['Rating']."<br><strong>Skill Level : </strong>".$r['SkillLevel']."<br><strong>Cook Time : </strong>".$r['CookTime']."m<br><strong>Prep Time : </strong>".$r['PrepTime']."m</p>	
                            </div>
                        </div>
                    </div>
                </div>
